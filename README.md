...
Так вот привилегии начинаются с браузера, который имеет свой IP формата указанного выше и этот браузер попав к вам на машину становится вашей собственностью, этот адрес ему назначает сетевая карта,она своеобразный домен имён для всех наших приложений и всё, что нам нужно, произвести запрос от имени этого адреса в обход того маршрута, что нам выдали маркетологи. Мы можем завести свой DNS в котором будем хранить все интересующие нас адреса, а самые убогие блокировать,почти так же, как это делают они с нами. 
Но не нужно мнить себя богом иначе мы потеряем связность, как теряли её они.По крайней мере 2 адреса из сети провайдера нам необходимы для получения белого ip в глобальной сети, в остальном мы создаём свой виртуальный сервер-хост как альтернативу ограниченному в правах.Подробнее об этом пространстве https://github.com/saminiir/level-ip 
Любой блокированный контент при помощи прокси мы можем ловить на свою локаль вот такого например адреса 0.0.0.0:57073 используя реверс запроса с нашего сокета,к браузеру и посреднику, вместо его запроса к браузеру и убогому IP . Тоже самое происходит в Tor но с использованием криптографии и отличных протоколов , которая иначе описывает адреса этой сети, со своими маршрутами и доменами.
